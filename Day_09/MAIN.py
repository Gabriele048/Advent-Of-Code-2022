...
# def nearToHead(headPos, tailPos):
#     if (headPos[0] - 1 <= tailPos[0] <=headPos[0] + 1) and (headPos[1]-1 <= tailPos[1] <= headPos[1] + 1):
...
#     if direction == "R":
#         for x in range(numberOfMove):
#             headPos[0] += 1
#             if not nearToHead(headPos, tailPos):
#                 tailPos[0] = headPos[0] - 1
#                 tailPos[1] = headPos[1]
#                 tailVisited.append([tailPos[0],tailPos[1]])
#     elif direction == "U":
#         for y in range(numberOfMove):
#             headPos[1] -= 1
#             if not nearToHead(headPos, tailPos):
...
# #print(tailVisited)
# atLeastOnceVisited = []
# for c in tailVisited:
#     if c not in atLeastOnceVisited:
#         atLeastOnceVisited.append(c)
# print(len(tailVisited))
# print(len(atLeastOnceVisited))
# inputFile.close()

#PART2
import numpy as np

moves = [el.split() for el in open('input.txt', 'r').readlines()]
